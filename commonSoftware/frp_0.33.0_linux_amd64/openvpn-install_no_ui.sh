...
WantedBy=multi-user.target" >/etc/systemd/system/iptables-openvpn.service
	...
	CLENT_CONNECT_IP="172.18.19.147"
...
#LAN-ONLY file
echo "client" >/etc/openvpn/client-lan-template.txt
	if [[ $PROTOCOL == 'udp' ]]; then
		echo "proto udp" >>/etc/openvpn/client-lan-template.txt
		echo "explicit-exit-notify" >>/etc/openvpn/client-lan-template.txt
	elif [[ $PROTOCOL == 'tcp' ]]; then
		echo "proto tcp-client" >>/etc/openvpn/client-lan-template.txt
	fi
CLIIP_IN_LAN=10.42.0.1
	echo "remote  $CLIIP_IN_LAN $VPNPORT
dev tun
resolv-retry infinite
nobind
persist-key
...
#ignore-unknown-option block-outside-dns
#setenv opt block-outside-dns # Prevent Windows 10 DNS leak
verb 3" >>/etc/openvpn/client-lan-template.txt
	if [[ $COMPRESSION_ENABLED == "y" ]]; then
		echo "compress $COMPRESSION_ALG" >>/etc/openvpn/client-template.txt
	fi
	# creat frpc_vpn.ini
	echo "[common]
server_addr = 172.18.19.147
server_port = 7000" >>/etc/openvpn/frpc_vpn.ini
    echo "[vpn_port_udp_$VPNPORT]
type = udp
local_port = 1194
remote_port = $VPNPORT" >> /etc/openvpn/frpc_vpn.ini
	  echo "[vpn_port_tcp_$VPNPORT]
type = tcp
local_port = 1194
remote_port = $VPNPORT" >> /etc/openvpn/frpc_vpn.ini
...
function newClientLan() {
	...
	# Generates the custom client.ovpn from lan example
	cp /etc/openvpn/client-lan-template.txt "$homeDir/$CLIENT.ovpn"
	{
		echo "<ca>"
